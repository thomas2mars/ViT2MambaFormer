...
# --- IMPORTS ---
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))   # distillation_b_wt_WK_only/ for WK.*
...
def load_student(cfg, teacher_state_dict, device, local_rank):
    """Initialize student from teacher weights (same as MO step init), skipping MO+HA pre-training.
    Non-Mamba weights (stem, LN, MLP, head, CLS tokens) are copied from the teacher.
    Mamba mixer weights start from random initialization.
    The same parameters as the standard WK step are unfrozen for training.
    """
    student = MambaFormer_Base_expand1_light_BiMamba2(double_cls_token=cfg.double_cls_token, image_size=384)

    # Initialize non-Mamba weights from teacher (same as MO step)
    ...
    print("Initialized student from teacher weights (MO-style). Mamba weights are random.")
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config',
                        help='Config module name under WK/ (e.g., config)')
    ...
    if is_master:
        ...
        print(f"=== WK-only: End-to-End Logit Distillation (no MO+HA pre-training) ===")
        print(f"World size: {world_size} | Batch per GPU: {cfg.base_batch_size} | Effective batch: {cfg.base_batch_size * world_size}")
        print(f"LR: {learning_rate:.2e} (base {cfg.base_lr:.2e} × {world_size}) | Warmup: {cfg.warmup_epochs} epochs | Temperature: {cfg.distillation_temp}")

    # --- Models ---
    teacher, teacher_state_dict = load_teacher(cfg, device)
    student = load_student(cfg, teacher_state_dict, device, local_rank)
