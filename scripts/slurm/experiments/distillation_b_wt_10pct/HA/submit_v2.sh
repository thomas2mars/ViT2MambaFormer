#!/bin/bash
#SBATCH --job-name=distill_HA_base_10pct_v2
...
#SBATCH --output=distillation_b_wt_10pct/HA/logs/%x_%j.out
#SBATCH --error=distillation_b_wt_10pct/HA/logs/%x_%j.err
...
mkdir -p distillation_b_wt_10pct/HA/logs

export OMP_NUM_THREADS=$SLURM_CPUS_PER_TASK

# Swap config to v2 for this run
cp distillation_b_wt_10pct/HA/config.py distillation_b_wt_10pct/HA/config.py.bak
cp distillation_b_wt_10pct/HA/config_v2.py distillation_b_wt_10pct/HA/config.py
...
torchrun --nproc_per_node=2 \
    distillation_b_wt_10pct/HA/main.py \
    --data_dir $SLURM_TMPDIR/ImageNet \
    $RESUME_FLAG

# Restore original config
mv distillation_b_wt_10pct/HA/config.py.bak distillation_b_wt_10pct/HA/config.py
